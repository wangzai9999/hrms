...
public interface TrainingApplyFoDao {
    ...
    @Update("update training_apply_fo set tf_auditor=#{tf_auditor},tf_status=#{tf_status},tf_opinion=#{tf_opinion} where tf_id=#{tf_id}")
    ...
    @Results(value = {
            ...
            @Result(column = "tf_tr_id",property = "tf_tr_id",one = @One(select = "com.dao.TrainingInfoDao.getOne",fetchType = FetchType.EAGER))
    },id = "td")
    @Select("select * from training_apply_fo where tf_id=#{id}")
    public TrainingApplyFo getOne(Long id);

    @ResultMap("td")
    @Select("<script>select * from (select t.*,rownum r from(select * from training_apply_fo " +
            "<where> <if test='param3.tf_id!=null and param3.tf_tr_id!=0'> and tf_id=#{2.tf_id}</if>" +
            " <if test='param3.tf_status!=null and param3.tf_status!=\"\"'> and tf_status=#{2.tf_status} </if> " +
            "</where>order by tf_id asc)t)a where a.r>(#{0}-1)*#{1} and a.r&lt;=#{0}*#{1} </script>")
    ...
    @Select("<script> select count(1) from training_apply_fo <where>" +
            " <if test='tf_id!=null and tf_id!=0'> and tf_tr_id=#{tf_id}</if>" +
    ...
    @ResultMap("td")
    @Select("select * from training_apply_fo where tf_us_id =#{id}")
